...
$backupFile = Join-Path $BackupFolder 'World\WorldOccupancyService.cs'
if (-not (Test-Path $backupFile)) {
    throw ('Backup file not found: ' + $backupFile)
}

Copy-Item $backupFile (Join-Path $root 'World\WorldOccupancyService.cs') -Force
Remove-Item -Recurse -Force (Join-Path $root 'bin'), (Join-Path $root 'obj') -ErrorAction SilentlyContinue

Write-Host 'Pass 8B source rollback complete.' -ForegroundColor Yellow
