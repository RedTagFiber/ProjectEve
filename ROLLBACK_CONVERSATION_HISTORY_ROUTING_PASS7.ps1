...
$backupFile = Join-Path $BackupFolder 'Conversations\ConversationManager.cs'
if (-not (Test-Path $backupFile)) {
    throw ('Backup file not found: ' + $backupFile)
}

Copy-Item $backupFile (Join-Path $root 'Conversations\ConversationManager.cs') -Force

Remove-Item -Recurse -Force (Join-Path $root 'bin'), (Join-Path $root 'obj') -ErrorAction SilentlyContinue

Write-Host 'Pass 7 source rollback complete.' -ForegroundColor Yellow
